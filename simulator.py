...
def run_algorithm(algorithm_func, algorithm_name, cities, output_file):
    start_time = time.perf_counter()
    tour, distance = algorithm_func(cities)
...
def run_tsp_simulation(cities, algorithms, output_file_path):
    ...
    # cities_exact = cities_approx = cities_heuristic = cities
    ...
    #     exact_results = held_karp_exact_tsp.run(cities)
    ...
    #     approx_results = christofides_approx_tsp.run(cities)
    ...
    #     heuristic_results = lin_kernighan_heuristic_tsp.run(cities)
    ...
    with open(output_file_path, "a") as output_file:
        tsp_runtimes = []
        algorithms_map = {
            "exact": (held_karp_exact_tsp.run, "Held-Karp"),
            "approx": (christofides_approx_tsp.run, "Christofides"),
            "heuristic": (lin_kernighan_heuristic_tsp.run, "Lin-Kernighan")
        }
        
        for algorithm_name in algorithms:
            algorithm_func, display_name = algorithms_map[algorithm_name]
            tsp_runtimes.append(run_algorithm(algorithm_func, algorithm_name, cities, output_file))
